...
then
    ...
    echo "Example: bash install_opencv.sh --4.5.5 --cpu"
    exit
fi
...
sudo apt-get install -y \
    build-essential cmake g++ wget unzip pkg-config gfortran checkinstall
...
wget -O opencv.zip https://github.com/opencv/opencv_contrib/archive/${CV_VERSION}.zip
unzip opencv.zip
rm opencv.zip
...
if [[ $UNIT == "GPU" || $UNIT == "gpu" ]]; then
    cmake -D CMAKE_BUILD_TYPE=RELEASE -D CMAKE_INSTALL_PREFIX=/opt/opencv/opencv_${CV_VERSION} \
        -D OPENCV_EXTRA_MODULES_PATH=~/opencv_contrib-${CV_VERSION}/modules -D OPENCV_PYTHON3_INSTALL_PATH=/opt/opencv/opencv_${CV_VERSION}/lib/python3.8/dist-packages \
        -D OPENCV_GENERATE_PKGCONFIG=ON -D BUILD_EXAMPLES=ON -D BUILD_opencv_cudaarithm=ON -D BUILD_opencv_cudabgsegm=ON -D BUILD_opencv_cudacodec=ON \
        ...
        -D BUILD_opencv_cudev=ON -D BUILD_opencv_superres=ON -D BUILD_opencv_videostab=ON -D WITH_CUBLAS=ON -D WITH_CUDA=ON \
        -D WITH_CUFFT=ON -D WITH_GPHOTO2=ON -D WITH_GIGEAPI=ON -D WITH_INTELPERC=OFF -D WITH_IPP_A=OFF -D WITH_LIBV4L=ON \
        -D WITH_LIBREALSENSE=ON -D WITH_OPENCLAMDBLAS=OFF -D WITH_OPENCLAMDFFT=OFF -D WITH_OPENGL=ON -D WITH_PTHREADS_PF=OFF \
        -D WITH_PVAPI=ON -D WITH_QT=ON -D WITH_TBB=ON -D WITH_UNICAP=OFF -D WITH_VTK=ON -D WITH_WEBP=ON -D CUDA_NVCC_FLAGS=--expt-relaxed-constexpr \
        -D CUDA_FAST_MATH=ON -D CUDA_TOOLKIT_ROOT_DIR=/usr/local/cuda -D CUDA_HOST_COMPILER=/usr/bin/gcc-9 \
        -D PYTHON_DEFAULT_EXECUTABLE=$(which python3) -D INSTALL_C_EXAMPLES=ON -D INSTALL_PYTHON_EXAMPLES=ON ..
    ...
    cmake \
        ...
        -D OPENCV_PYTHON3_INSTALL_PATH=/opt/opencv/opencv_${CV_VERSION}/lib/python3.8/dist-packages \
...
sudo mv /opt/opencv/opencv_${CV_VERSION}/lib/python3.8/dist-packages/cv2/python-3.8/cv2.cpython-38-x86_64-linux-gnu.so /opt/opencv/opencv_${CV_VERSION}/lib/python3.8/dist-packages/cv2.so
sudo ln -s /opt/opencv/opencv_${CV_VERSION}/lib/python3.8/dist-packages/cv2.so /usr/local/lib/python3.8/dist-packages/cv2.so


sudo rm ~/opencv-${CV_VERSION} -r
sudo rm ~/opencv_contrib-${CV_VERSION} -r
