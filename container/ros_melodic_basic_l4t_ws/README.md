...
`ros_melodic_basic_l4t_ws`コンテナは以下の環境で構築されます．
...
また，コンテナ内の `catkin_ws/src` は， `ros_melodic_basic_l4t_ws/src` と接続されています．(run.shを参照) 
...
1. コンテナのフォルダをコピします．
```bash
$ cp -r {docker_wsのPATH}/container/ros_melodic_basic_l4t_ws/ {コンテナPATH}
# 例: $ cp -r ~/docker_ws/container/ros_melodic_basic_l4t_ws/ ~/
```
> **Warning**
> コピされたフォルダの名前を変えてください．
> 例: `ros_melodic_basic_l4t_ws` → `my_new_ws`
