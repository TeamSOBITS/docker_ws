...
`base_1804_ws`コンテナは以下の環境で構築されます．
...
また，コンテナ内の `catkin_ws/src` は， `base_1804_ws/src` と接続されています．(run.shを参照) 
...
1. コンテナのフォルダをコピします．
```bash
$ cp -r {docker_wsのPATH}/container/base_1804_ws/ {コンテナPATH}
# 例: $ cp -r ~/docker_ws/container/base_1804_ws/ ~/
```
> **Warning**
> コピされたフォルダの名前を変えてください．
> 例: `base_1804_ws` → `my_new_ws`
