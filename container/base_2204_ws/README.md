...
1. Dockerfileがある階層まで移動(例としてCUI操作のcpu版の実行手順を示します)
    ```
    cd ~/docker_ws/container/base_2204_ws/Dockerfiles/cpu
    ```
...
#コンテナの停止
docker stop base_2204_ws  #docker stop <CONTAINER NAME or CONTAINER ID>

#コンテナの再起動
docker start base_2204_ws #docker start <CONTAINER NAME or CONTAINER ID>

#コンテナの削除(起動中のコンテナは削除できない)
docker rm base_2204_ws #docker rm <CONTAINER NAME or CONTAINER ID>

#イメージの削除(コンテナが残っている場合は削除できない)
docker rmi sobits/base_2204_ws #docker rmi <IMAGE NAME or IMAGE ID>

```

コンテナ内の catkin_ws/src は、base_2204_ws/src とリンクするように設定しています（run.shを参照）。  
