...
`ros_melodic_sobits_l4t_ws`コンテナは以下の環境で構築されます．
- Ubuntu : 18.04  (L4T R32.7.1)
- JetPack : 4.6.1
- ROS : Melodic Morenia
- OpenCV : 4.6.0 (2021-12-30)
- Python : 2.7 & 3.6 (デフォルト)
- PyTorch : 1.10.0
    - torchvision : 0.11.0
    - torchaudio : 0.10.0
- UserName : sobits
...
また，コンテナ内の `catkin_ws/src` は， `ros_melodic_sobits_l4t_ws/src` と接続されています．(run.shを参照) 
...
1. コンテナのフォルダをコピします．
```bash
$ cp -r {docker_wsのPATH}/container/ros_melodic_sobits_l4t_ws/ {コンテナPATH}
# 例: $ cp -r ~/docker_ws/container/ros_melodic_sobits_l4t_ws/ ~/
```
> **Warning**
> コピされたフォルダの名前を変えてください．
> 例: `ros_melodic_sobits_l4t_ws` → `my_new_ws`
