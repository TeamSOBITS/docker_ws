...
`base_2204_ws`コンテナは以下の環境で構築されます．
...
また，コンテナ内の `colcon_ws/src` は， `base_2204_ws/src` と接続されています．(run.shを参照) 
...
1. コンテナのフォルダをコピします．
```bash
$ cp -r {docker_wsのPATH}/container/base_2204_ws/ {コンテナPATH}
# 例: $ cp -r ~/docker_ws/container/base_2204_ws/ ~/
```
> **Warning**
> コピされたフォルダの名前を変えてください．
> 例: `base_2204_ws` → `my_new_ws`
